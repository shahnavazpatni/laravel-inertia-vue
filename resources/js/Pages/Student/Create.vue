...
// Watch the entire form object and clear errors dynamically
watch(
    () => form.data(),
    (newForm) => {
        ...
        // Trigger getSections when class_id changes
        if (newForm.class_id) {
            getSections(newForm.class_id);
        }
    },
    { deep: true }
);
...
const submit = () => {
    form.post(route("students.store"), {
        ...
        onSuccess: () => {
            ...
            // Trigger toast after successful form submission
            if (toastRef.value) {
                toastRef.value.show();
            }
        },
...
<template>
    ...
    <Toast ref="toastRef" message="Student created successfully!" />
